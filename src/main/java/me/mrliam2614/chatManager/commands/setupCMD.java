...
public class setupCMD implements Listener {
    ...
    public void prefix(CommandSender sender, String[] args) {
        if (ConfigVariable.MySqlEnable) {
            ...
            sender.sendMessage(ConfigVariable.prefixUpdated.replace("{GROUP}", args[2]).replace("{PREFIX}", plugin.cc(args[3])));
        } else {
    ...
    public void suffix(CommandSender sender, String[] args) {
        if (ConfigVariable.MySqlEnable) {
            ...
            sender.sendMessage(ConfigVariable.suffixUpdated.replace("{GROUP}", args[2]).replace("{SUFFIX}", plugin.cc(args[3])));
        } else {
